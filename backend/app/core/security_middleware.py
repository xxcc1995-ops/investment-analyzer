...
# 需要认证的敏感路由前缀（交易、组合管理、拖拉机等写操作密集的模块）
SENSITIVE_PREFIXES = (
    "/api/tractor",
    "/api/portfolio",
    "/api/futu-options",
...
class ApiKeyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.warning("API Key 认证失败: path=%s, ip=%s", path, request.client.host if request.client else "unknown")
        return JSONResponse(
            status_code=401,
