...
class CryptoMasterService:
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                result = {
                    ...
                    "btc_24h_change": round(btc_data.get("bitcoin", {}).get("usd_24h_change", 0), 2),
                    "eth_price": btc_data.get("ethereum", {}).get("usd", 0),
                    "eth_24h_change": round(btc_data.get("ethereum", {}).get("usd_24h_change", 0), 2),
    ...
    async def _get_fear_greed(self) -> dict:
        ...
        try:
            ...
            return {"value": 50, "label": "N/A", "update_time": ""}

    async def get_top_coins(self, limit: int = 50) -> dict:
        """Top N加密货币排行"""
    ...
    async def get_trending(self) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                res = await client.get("https://api.coingecko.com/api/v3/search/trending")
                data = res.json()
    ...
    async def get_stablecoin_monitor(self) -> dict:
        """稳定币市值监控"""
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                res = await client.get("https://stablecoins.llama.fi/stablecoins?includePrices=true")
                data = res.json()
    ...
    async def get_btc_dominance_history(self) -> dict:
        """BTC主导率变化（通过CoinGecko global数据获取当前值）"""
        try:
    ...
    async def get_defi_tvl(self) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                # 各链TVL
                chains_res = await client.get("https://api.llama.fi/v2/chains")
    ...
    async def get_chain_comparison(self) -> dict:
        """公链对比数据"""
            ...
            "beginner": {
                ...
                "sections": [
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "ETH是以太坊的原生代币，用于支付'Gas费'",
                            "智能合约 = 自动执行的'如果...那么...'规则",
                            "ERC-20标准让任何人都能发行代币",
                            "2022年从PoW转向PoS（合并），能耗降低99.95%",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "CEX（如Binance、OKX、Coinbase）：像股票交易所，注册即可交易",
                            "DEX（如Uniswap、PancakeSwap）：无需注册，用钱包直接交易",
                            "现货交易 vs 合约交易：现货是买卖实物，合约是押注涨跌",
                            "一定要开启2FA双重验证！",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "热钱包（联网）：MetaMask、Trust Wallet，方便但安全性较低",
                            "冷钱包（离线）：Ledger、Trezor，安全性最高",
                            "助记词（12/24个单词）= 私钥的备份，写在纸上！绝不要截图或存在网络上",
                            "'Not your keys, not your coins'——不在交易所放太多资产",
                        ],
                ...
                "sections": [
                    {
                        ...
                        "key_points": [
                            "K线图：开盘价、收盘价、最高价、最低价",
                            "支撑位/阻力位：价格反复测试但难以突破的位置",
                            "移动平均线(MA)：5日/20日/60日/200日均线",
                            "成交量：放量突破比缩量突破更可靠",
                            "RSI指标：>70超买，<30超卖",
                            "MACD：金叉买入信号，死叉卖出信号",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "交易所净流入/流出：大量流入可能预示抛售，流出可能预示囤币",
                            "巨鲸动向：大额转账往往预示大行情",
                            "活跃地址数：网络使用率的直接指标",
                            "MVRV比率：市场价值/实现价值，>3.5通常过热，<1通常底部",
                            "SOPR：花费产出利润率，>1盈利卖出，<1亏损卖出",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "团队背景：创始人是否有技术背景和行业经验？",
                            "代币经济学(Tokenomics)：总量、分配、解锁计划、销毁机制",
                            "TVL（总锁仓量）：DeFi协议的真实使用量",
                            "开发者活跃度：GitHub提交频率",
                            "社区治理：是否有去中心化治理？",
                            "竞争对手对比：同赛道项目横向比较",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "比特币减半周期：历史上减半后12-18个月通常迎来牛市",
                            "恐惧贪婪指数：极度恐惧时买入，极度贪婪时卖出",
                            "资金费率：合约市场多空比，极端值往往预示反转",
                            "稳定币市值增长 = 新资金入场",
                            "FOMO（害怕错过）是最大的敌人",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            ...
                            "借贷协议：Aave、Compound——存币赚利息，抵押借币",
                            "流动性挖矿：提供流动性赚取交易手续费+代币奖励",
                            "质押(Staking)：锁定代币获得网络奖励",
                            "稳定币：USDT/USDC/DAI——加密世界的'现金'",
                        ],
                ...
                "sections": [
                    {
                        ...
                        "key_points": [
                            "凯利公式：f* = (bp - q) / b，计算最优仓位比例",
                            "单笔风险不超过总资金的2%",
                            "金字塔加仓：初始仓位最大，越涨加仓越少",
                            "永远留有现金（至少20-30%），应对黑天鹅",
                            "不要把所有鸡蛋放在一个篮子里",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "固定比例止损：亏损达到预设比例（如-8%）无条件止损",
                            "移动止损：价格上涨后止损位也跟着上移",
                            "止盈分批：到达目标位先卖一半，剩下的用移动止损保护",
                            "不要移动止损去承受更大亏损！",
                            "止损是成本，不是失败。每笔交易都要先想好止损位。",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "跨交易所套利：同一币种在不同交易所的价差",
                            "期现套利：期货溢价/折价时做多空对冲",
                            "三角套利：利用三种币之间的汇率差",
                            "稳定币脱锚套利：USDT短暂偏离1美元时的操作",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "每个牛市都有主题叙事：2017 ICO、2021 DeFi/NFT、2024 AI/RWA",
                            "早期发现叙事：关注VC投资方向、开发者趋势、社交媒体热度",
                            "叙事轮动：资金会在不同叙事间流动",
                            "不要爱上叙事，叙事结束要及时离场",
                        ],
                    ...
                    },
                    {
                        ...
                        "key_points": [
                            "熊市底部：60% BTC + 30% ETH + 10% 现金",
                            "牛市初期：40% BTC + 30% ETH + 20% 大盘山寨 + 10% 现金",
                            "牛市中期：30% BTC + 20% ETH + 30% 中小盘山寨 + 10% Meme + 10% 现金",
                            "牛市末期：逐步减仓，增加现金和稳定币比例",
                            "绝对不要满仓！留子弹应对意外。",
                        ],
                    },
                ],
    ...
    def get_learning_path(self) -> dict:
        ...
        return {
            "phases": [
                ...
                },
                {
                    ...
                    "tasks": [
                        ...
                        "了解市值、流通量、交易量的含义",
                        "关注行业新闻和社交媒体",
                        "了解不同赛道：Layer1、DeFi、NFT、Meme等",
                        "学会使用CoinGecko/CoinMarketCap",
                        "开始记录交易日志",
                    ],
            ...
            "golden_rules": [
                "永远不要投资超过你能承受全部亏损的金额",
                "DYOR（Do Your Own Research）——自己研究，不要盲目跟单",
                "保护本金是第一要务",
                "不要借钱/贷款炒币",
                "保持学习，市场永远在变化",
            ],
    ...
    def simulate_dca(self, coin: str, monthly_amount: float, months: int) -> dict:
        ...
        for i in range(months):
            # 模拟加密市场的高波动性
            base_return = random.gauss(0.02, 0.15)  # 月均2%收益，15%标准差
            monthly_returns.append(base_return)
        ...
        for i, ret in enumerate(monthly_returns):
            ...
            history.append({
                "month": i + 1,
                "invested": monthly_amount * (i + 1),
                "value": round(total_value, 2),
                "return_pct": round((total_value / (monthly_amount * (i + 1)) - 1) * 100, 2),
        ...
        return {
            ...
            "avg_cost": round(total_invested / total_coins, 2) if total_coins > 0 else 0,
            "history": history,
            "note": "⚠️ 以上为模拟数据，仅供学习理解定投原理。实际收益取决于真实市场价格。",
        }
    ...
    def calculate_position(self, total_capital: float, risk_per_trade: float,
                           win_rate: float, avg_win: float, avg_loss: float) -> dict:
        """仓位计算 - Kelly公式 + 固定风险"""
        # Kelly公式
        ...
        return {
            ...
            "formula": {
                "kelly": "f* = (bp - q) / b",
                "where": "b=赔率(平均盈利/平均亏损), p=胜率, q=败率(1-p)",
            },
        }
    ...
    def _interpret_position(self, kelly: float, half_kelly: float, expected: float) -> str:
        ...
        parts.append("💡 实际操作建议使用半Kelly（Half-Kelly），降低波动性。")
        return " ".join(parts)
            ...
            "mistakes": [
                {
                    "name": "FOMO追高",
                    "frequency": "⭐⭐⭐⭐⭐",
                    "description": "看到别人赚钱就冲动买入，通常在最高点附近入场",
                    "solution": "设定规则：大涨的币不追，等回调再买。或者用定投平滑成本。",
                    "example": "2021年11月BTC $69000时大量新手入场，之后跌到$16000",
                },
                {
                    "name": "恐慌割肉",
                    "frequency": "⭐⭐⭐⭐⭐",
                    "description": "价格暴跌时恐慌卖出，卖在最低点",
                    "solution": "提前设好止损位，用限价单自动执行。不要在大跌时做决定。",
                    "example": "2020年3月12日BTC一天跌50%，恐慌卖出的人错过了之后从$4000到$69000的涨幅",
                },
                {
                    "name": "梭哈山寨币",
                    "frequency": "⭐⭐⭐⭐",
                    "description": "把所有资金投入一个山寨币，期望百倍收益",
                    "solution": "核心仓位（60-70%）放在BTC和ETH，山寨币用小仓位。",
                    "example": "99%的山寨币最终归零，只有极少数能存活一个完整周期",
                },
                {
                    "name": "合约爆仓",
                    "frequency": "⭐⭐⭐⭐",
                    "description": "用高杠杆玩合约，一次反向波动就爆仓",
                    "solution": "新手远离合约！如果一定要玩，杠杆不超过3倍，仓位不超过总资金10%。",
                    "example": "2021年5月19日，全网合约爆仓超过100亿美元",
                },
                {
                    "name": "沉没成本",
                    "frequency": "⭐⭐⭐",
                    "description": "亏损后不断加仓'摊低成本'，越套越深",
                    "solution": "亏损的仓位不加仓。止损出局，等信号明确再重新入场。",
                    "example": "LUNA从$100跌到$50时很多人抄底，最终归零",
                },
                {
                    "name": "听信KOL",
                    "frequency": "⭐⭐⭐⭐",
                    "description": "盲目跟单推特/群里的'老师'，不做自己的研究",
                    "solution": "DYOR！别人的分析可以参考，但决策必须自己做。",
                    "example": "很多KOL收钱推广垃圾项目，粉丝接盘后暴跌",
                },
                {
                    "name": "频繁交易",
                    "frequency": "⭐⭐⭐",
                    "description": "每天都想交易，手续费吃掉利润",
                    "solution": "减少交易频率，每次交易都要有明确的理由和计划。",
                    "example": "每天交易一次，每次0.1%手续费，一年下来手续费就吃掉36%的本金",
                },
                {
                    "name": "忽视安全",
                    "frequency": "⭐⭐⭐",
                    "description": "不重视钱包安全，被盗或丢失私钥",
                    "solution": "启用2FA、用冷钱包存大额资产、备份助记词到安全的地方。",
                    "example": "FTX暴雷时，放在交易所的数十亿美元资产被冻结",
                },
            ],
        }
    ...
    def get_security_guide(self) -> dict:
        ...
        return {
            ...
            "exchange_security": {
                "title": "交易所安全",
                "rules": [
                    "一定要开启Google Authenticator（不要用短信验证）",
                    "设置提币白名单",
                    "不要在交易所放超过总资产的30%",
                    "选择头部交易所（Binance、OKX、Coinbase）",
                    "FTX的教训：再大的交易所也可能暴雷",
                ],
            },
            "scam_prevention": {
                ...
                "rules": [
                    ...
                    "免费空投要你先转账 = 100%骗局",
                    "高回报无风险 = 100%骗局",
                    "不要点击不明链接，尤其是Discord/Telegram里的",
                    "验证合约地址：只从官方渠道获取",
                    "貔貅合约：只能买不能卖的代币，用Token Sniffer检查",
                ],
            },
        }
    ...
    def get_defi_guide(self) -> dict:
        ...
        return {
            "levels": [
                {
                    ...
                    "name": "DeFi初体验",
                    "protocols": [
                        {"name": "Uniswap", "type": "DEX", "chain": "Ethereum/Arbitrum/Base", "risk": "低",
                         "action": "用ETH交换一个小额代币，体验DEX交易"},
                        {"name": "Aave", "type": "借贷", "chain": "Ethereum/Arbitrum", "risk": "低",
                         "action": "存入USDC赚取利息，了解借贷机制"},
                    ],
                ...
                },
                {
                    ...
                    "name": "流动性提供",
                    "protocols": [
                        {"name": "Curve", "type": "稳定币DEX", "chain": "多链", "risk": "低-中",
                         "action": "为稳定币池提供流动性，赚取手续费"},
                        {"name": "Lido", "type": "质押", "chain": "Ethereum", "risk": "低",
                         "action": "质押ETH获得stETH，赚取质押收益"},
                    ],
                ...
                },
                {
                    ...
                    "name": "收益优化",
                    "protocols": [
                        {"name": "Pendle", "type": "收益代币化", "chain": "多链", "risk": "中",
                         "action": "分离资产的本金和收益部分进行交易"},
                        {"name": "Ethena", "type": "合成美元", "chain": "Ethereum", "risk": "中-高",
                         "action": "了解delta中性策略和sUSDe收益"},
                    ],
            ...
            "gas_optimization": {
                "title": "Gas费优化",
                "tips": [
                    "使用Layer2（Arbitrum、Optimism、Base）降低Gas费90%+",
                    "在Gas费低的时段操作（UTC凌晨/周末）",
                    "批量操作：DeFi Saver等工具可以批量执行交易",
                    "设置Gas上限，避免意外高Gas费",
                ],
            },
        }
    ...
    def get_airdrop_guide(self) -> dict:
        ...
        return {
            ...
            "how_it_works": [
                "项目方确定空投资格标准（使用次数、TVL贡献、时间等）",
                "快照：记录符合条件的地址",
                "开放领取：符合条件的用户领取免费代币",
            ],
            "strategies": [
                {"name": "多链交互", "description": "在多条链上使用主流协议，增加被快照的机会",
                 "effort": "中", "potential": "高"},
                {"name": "测试网参与", "description": "参与新项目的测试网，通常成本为零",
                 "effort": "低", "potential": "中"},
                {"name": "治理参与", "description": "参与DAO治理投票，展示长期社区参与",
                 "effort": "低", "potential": "中"},
                {"name": "Gitcoin捐赠", "description": "通过Gitcoin等平台捐赠，获得多个项目空投资格",
                 "effort": "低", "potential": "中-高"},
            ],
            "risk_warning": [
                "不要为了空投投入大量资金——空投不是保证的",
                "注意Gas费成本，确保交互成本低于潜在空投价值",
                "小心钓鱼网站——只从官方渠道获取信息",
                "空投代币通常会先涨后跌，考虑领取后立即卖出一部分",
            ],
            "tools": [
    ...
    def get_master_wisdom(self) -> dict:
        ...
        return {
            "wisdom": [
                {
                    "person": "中本聪 (Satoshi Nakamoto)",
                    "role": "比特币创始人",
                    "quotes": [
                        "如果你不相信我或者不理解它，我没有时间去说服你。",
                        "我对更安全的基于哈希的工作量证明没有更好的替代方案。",
                    ],
                    "lesson": "技术信仰。真正的创新不需要说服所有人。",
                },
                {
                    "person": "Vitalik Buterin",
                    "role": "以太坊创始人",
                    "quotes": [
                        "加密货币的目标不是让少数人变得极其富有，而是让很多人获得经济自由。",
                        "不要投资你不懂的东西。",
                    ],
                    "lesson": "技术理想主义。投资前先理解技术。",
                },
                {
                    "person": "CZ (赵长鹏)",
                    "role": "Binance创始人",
                    "quotes": [
                        "FUD是你的朋友，当别人恐惧时你应该贪婪。",
                        "保持饥饿，保持愚蠢。",
                    ],
                    "lesson": "逆向思维。市场恐慌时往往是最好的机会。",
                },
                {
                    "person": "Arthur Hayes",
                    "role": "BitMEX创始人",
                ...
                {
                    "person": "Su Zhu",
                    "role": "Three Arrows Capital创始人",
                    "quotes": [
                        "超级周期理论：加密货币将不断创出新高。",
                    ],
                    "lesson": "⚠️ 反面教材：再有影响力的观点也可能是错的。3AC在2022年破产。永远保持独立思考。",
                },
                {
                    "person": "加密社区",
                    "role": "集体智慧",
                    "quotes": [
                        ...
                        "WAGMI (We're All Gonna Make It) — 我们都会成功的",
                        "Not your keys, not your coins — 不是你的私钥，就不是你的币",
                        "Buy the dip — 逢低买入",
                        "HODL — 坚定持有",
                    ],
                    "lesson": "社区文化。这些格言背后有深刻的道理。",
                },
            ],
        }
